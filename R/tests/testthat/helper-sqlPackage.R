...
helper_cleanAllExternalLibraries <- function()
{
  names <- sqlmlutils:::execute(connection, "select * from sys.external_libraries")$name
  for(name in names)
  {
    sqlmlutils:::execute(connection, paste0("DROP EXTERNAL LIBRARY ", name))
  }
}
